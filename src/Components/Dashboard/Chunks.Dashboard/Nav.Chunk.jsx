                    ...
                    <ul className='flex flex-col gap-2 rounded-md overflow-auto h-full'>
                         {courses.length > 0 ? courses.map((course, index) => (
                              <NavLink
                                   ...
                                   className={`
                                        flex items-center flex-nowrap rounded-md gap-2 h-8 px-2
                                        ${openNav ? 'w-full justify-start' : 'w-8 justify-center'}
                                        ${({ isActive }) => isActive ? 'bg-[#452829]/20 border border-[#90594a]/70 text-yellow-200' : ''}
                                   `}
                                   style={({ isActive }) => isActive ? { background: "rgba(69,40,41,0.20)", fontWeight: 400, border: "2px solid #90594a", color: "#ffe991" } : undefined}
                              >
                                   <i className={`${course.icon} text-[1.23rem]`}></i>
                                   {openNav && <h2 className='text-nowrap poppins text-sm'>{course.title}</h2>}
                              </NavLink>
